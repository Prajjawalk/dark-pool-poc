 ...
 */
abstract contract DebtERC20 is IConfidentialERC20, IERC20Errors, TFHEErrors {
    ...
    function burn(einput encryptedAmount, bytes calldata inputProof) public virtual returns (bool) {
        burn(TFHE.asEuint64(encryptedAmount, inputProof));
        return true;
    }

    function burn(euint64 amount) public virtual returns (bool) {
        _isSenderAllowedForAmount(amount);

        /// @dev Make sure the owner has enough tokens.
        ebool canBurn = TFHE.le(amount, _balances[msg.sender]);
        euint64 burnValue = TFHE.select(canBurn, amount, TFHE.asEuint64(0));
        _unsafeBurn(msg.sender, burnValue);
        return true;
    }

    function burnFrom(address from, einput encryptedAmount, bytes calldata inputProof) public virtual returns (bool) {
        burnFrom(from, TFHE.asEuint64(encryptedAmount, inputProof));
        return true;
    }

    function burnFrom(address from, euint64 amount) public virtual returns (bool) {
        _isSenderAllowedForAmount(amount);
        address spender = msg.sender;
        ebool isBurnable = _updateAllowance(from, spender, amount);
        euint64 burnValue = TFHE.select(isBurnable, amount, TFHE.asEuint64(0));
        _unsafeBurn(from, burnValue);
        return true;
    }
     ...
     * Total supply is not stored hence returning 0
     */
     ...
     *      by the function calling it.
     */
    function _unsafeMintNoEvent(address account, euint64 amount) internal virtual {
     ...
     *      by the function calling it.
     */
    function _unsafeBurn(address account, euint64 amount) internal {
     ...
     *      by the function calling it.
     */
    function _unsafeBurnNoEvent(address account, euint64 amount) internal {
